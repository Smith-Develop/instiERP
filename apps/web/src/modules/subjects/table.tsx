...
export function SubjectTable({ subjects }: { subjects: Subject[] }) {
  ...
  async function handleDelete(id: string) { if(!confirm("¿Eliminar?")) return; setDeletingId(id); await fetch(`/api/subjects/${id}`,{method:"DELETE"}); router.refresh(); }
  return (
