  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-slate-200 bg-white">
        <table className="w-full"><thead><tr className="border-b bg-slate-50">{["Estudiante","Concepto","Importe","Vencimiento","Estado"].map(h=><th key={h} className="h-12 px-4 text-left text-xs font-medium uppercase text-slate-500">{h}</th>)}</tr></thead>
        <tbody>
          {invoices.length===0 ? <tr><td colSpan={5} className="p-8 text-center text-sm text-slate-400">No hay facturas</td></tr> :
            invoices.map(inv => (
