                        ...
                        <div className="flex flex-wrap gap-1">
                          {grade.sections.length === 0 ? (
                            ...
                            grade.sections.map(section => (
                              <span key={section.id} className="group relative inline-flex rounded bg-[#1E3A5F]/10 px-2 py-0.5 text-xs font-medium text-[#1E3A5F] hover:bg-[#1E3A5F]/20 hover:cursor-pointer transition-colors" onClick={() => setStudentsModal({ open: true, sectionId: section.id, sectionName: `${grade.name} ${section.name}` })}>
                                {section.name}
