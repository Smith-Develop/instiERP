...
export function AcademicLevelModal({ open, onClose, editId, editName }: { open: boolean; onClose: () => void; editId?: string; editName?: string }) {
  ...
  const mut = useMutation({
    mutationFn: async (data: FormData) => {
      const url = editId ? `/api/academic/levels/${editId}` : "/api/academic/levels";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-50 w-full max-w-sm rounded-lg border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b px-6 py-4"><h2 className="text-lg font-bold text-slate-900">{editId ? "Editar nivel" : "Nuevo nivel"}</h2><button onClick={onClose} className="rounded-md p-1 text-slate-400 hover:text-slate-600"><X className="h-5 w-5" /></button></div>
        <form onSubmit={handleSubmit(d => mut.mutate(d))} className="px-6 py-4 space-y-4">
          <div className="space-y-1.5"><Label>Nombre *</Label><Input {...register("name")} placeholder="Primaria, Secundaria, Bachillerato..." />{errors.name && <p className="text-xs text-red-600">{errors.name.message}</p>}</div>
