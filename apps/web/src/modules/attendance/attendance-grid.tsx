  ...
  const loadAttendance = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/attendance?sectionId=${sectionId}&date=${date}`,
      );
    ...
    setStudents((prev) =>
      prev.map((s) =>
        s.studentId === studentId
          ? { ...s, status: STATUS_CONFIG[s.status].next }
          : s,
      ),
    );
    setSaved(false);
    ...
    <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-end gap-4">
            <div className="space-y-2">
              ...
              <select
                ...
                className="flex h-10 w-full min-w-[220px] rounded-md border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#1E3A5F]"
              >
                {sections.map((s) => (
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="flex h-10 w-full min-w-[180px] rounded-md border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#1E3A5F]"
              />
            </div>
            <div className="flex items-end gap-2">
              ...
              <Button variant="ghost" onClick={() => markAll("AUSENTE")}>
                <X className="h-4 w-4" /> Todos ausentes
              </Button>
            </div>
          </div>
        </CardContent>
      ...
      {students.length > 0 && (
        <div className="flex flex-wrap gap-3 text-sm">
          <span className="inline-flex items-center gap-1 rounded-md border border-slate-200 px-3 py-1">
            <Users className="h-3.5 w-3.5 text-slate-400" />
            <span className="font-medium">{counts.total}</span> estudiantes
          </span>
          <span className="inline-flex items-center gap-1 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-1 text-emerald-700">
            ...
            {counts.presente} presente
          </span>
          <span className="inline-flex items-center gap-1 rounded-md border border-red-200 bg-red-50 px-3 py-1 text-red-700">
            <X className="h-3.5 w-3.5" />
            {counts.ausente} ausente
          </span>
          <span className="inline-flex items-center gap-1 rounded-md border border-amber-200 bg-amber-50 px-3 py-1 text-amber-700">
            <Clock className="h-3.5 w-3.5" />
            {counts.tardanza} tardanza
          </span>
          <span className="inline-flex items-center gap-1 rounded-md border border-blue-200 bg-blue-50 px-3 py-1 text-blue-700">
            <FileText className="h-3.5 w-3.5" />
            {counts.justificado} justificado
          </span>
        </div>
      )}
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="py-12 text-center text-sm text-slate-400">
              Selecciona una sección y fecha para ver la lista
            </div>
          ) : (
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {students.map((student) => (
                <button
                  ...
                  className={`flex items-center justify-between rounded-md border px-4 py-3 text-left transition-all active:scale-[0.98] ${STATUS_CONFIG[student.status].color}`}
